...
EXCEL_FILE = "bytebytego_blogs.xlsx"
...
def fetch_blog_data():
    ...
    elements = driver.find_elements(By.CSS_SELECTOR, 'div.container-Qnseki a[data-testid="post-preview-title"]')

    blog_entries = []
    for el in elements:
        title = el.text.strip()
        link = el.get_attribute("href").strip()
        if "EP" in title:
...
def update_excel(blog_entries):
    ...
    if new_entries:
        ...
        print(f"✅ Added {len(new_entries)} new blog(s) to {EXCEL_FILE}")
    else:
        print("🔄 No new blogs found.")

def main():
    blog_entries = fetch_blog_data()
